 ...
 *
 * Minor changes by AMR for compatibility with GTK+-2.0
 *
...
#else
#define _
#define g_filename_display_name g_strdup
...
typedef struct
{
  ...
  /* Optional Generic */
  long filesize;  // Change by AMR for GTK+-2.0 compatibility
  gchar *mime_type;
  gchar *description;
  gchar *software;
...
{
#ifdef WIN32
  if (!g_file_test (path, G_FILE_TEST_IS_DIR) && mkdir (path) < 0)
#else
...
#ifdef WIN32
//  g_return_val_if_fail (filename != NULL && (filename[1] == ':' || filename[1] == '\\'), NULL);
#else
...
// Change by AMR for GTK+-2.0 compatibility
      tmp = g_strdup_printf ("%ld", data->filesize);
